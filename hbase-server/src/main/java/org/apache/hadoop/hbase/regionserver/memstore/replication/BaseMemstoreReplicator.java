...
public abstract class BaseMemstoreReplicator implements MemstoreReplicator {
  ...
  static class EntryBuffers {
    ...
    public CompletedFuture appendEntry(MemstoreReplicationEntry entry, RegionLocations locations)
      ...
      /*
       * synchronized (controller.dataAvailable) { totalBuffered += incrHeap; while (totalBuffered >
       * maxHeapUsage && controller.thrown.get() == null) { LOG.debug( "Used " + totalBuffered +
       * " bytes of buffered edits, waiting for IO threads...");
       * controller.dataAvailable.wait(2000); } controller.dataAvailable.notifyAll(); }
       * controller.checkForErrors();
       */
      return future;
    }
    ...
    void doneWriting(RegionEntryBuffer buffer) {
      ...
      /*
       * synchronized (controller.dataAvailable) { totalBuffered -= size; // We may unblock writers
