...
public class TestRegionReplicasWith3ReplicasWithMultiCF {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
      HBaseClassTestRule.forClass(TestRegionReplicasWith3ReplicasWithMultiCF.class);
  private static final Log LOG = LogFactory.getLog(TestRegionReplicasWith3ReplicasWithMultiCF.class);
  ...
  public static void before() throws Exception {
    ...
    final TableName tableName = TableName.valueOf(TestRegionReplicasWith3ReplicasWithMultiCF.class.getSimpleName());

    // Create table then get the single region for our new table.
    HTableDescriptor htd = new HTableDescriptor(tableName);
    htd.setRegionReplication(3);
    table =
        HTU.createTable(htd, new byte[][] { f, f1 }, null, new Configuration(HTU.getConfiguration()));
    Thread.sleep(5000);
  ...
  public void testGetOnTargetRegionReplica() throws Exception {
    ...
    try {
     // pair = openSecondary();
      ...
      HTU.loadNumericRows(table, new byte[][]{f, f1}, 0, 100);
...
public LoadThread(Pair<OpenedIn, OpenedIn> pair) {
    ...
    public void run() {
      try {
        for(int i = 0; i < 100; i++) {
        HTU.loadNumericRows(table,  new byte[][]{f, f1}, 0, i);
        if(i % 10 == 0) {
          Region primaryRegion = getPrimaryRegion(pair);
  ...
  public void testVerifySecondaryAbilityToReadWithOnFiles() throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < 3; ++i) {
        HTU.loadNumericRows(table,  new byte[][]{f, f1}, i * 1000, (i + 1) * 1000);
  ...
  public void testRefreshStoreFiles() throws Exception {
    ...
    try {
      ...
      //load some data to primary
      LOG.info("Loading data to primary region");
      HTU.loadNumericRows(table,  new byte[][]{f, f1}, 0, 1000);
      // assert that we can read back from primary
      ...
      Thread.sleep(3000);
      // load some data to primary
      HTU.loadNumericRows(table,  new byte[][]{f, f1}, 1000, 1100);
      ((HRegion)primaryRegion).flush(true);

      HTU.loadNumericRows(table,  new byte[][]{f, f1}, 2000, 2100);
      ((HRegion)primaryRegion).flush(true);
