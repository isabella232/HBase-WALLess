...
public class TestRegionReplicaSplit {
  ...
  private static final Log LOG = LogFactory.getLog(TestRegionReplicaSplit.class);
  ...
  public void testRegionReplicaConvertedToPrimary() throws Exception {
    try {
      ...
      for (RegionServerThread rs : HTU.getMiniHBaseCluster().getRegionServerThreads()) {
        ...
        for (Region r : rs.getRegionServer().getRegions(table.getName())) {
          if (!regions.contains(r.getRegionInfo())
            ...
            System.out.println("The region is " + r.getRegionInfo() + " the location is "
                + rs.getRegionServer().getServerName());
      ...
      assertEquals("The new regions should be assigned to 2 unique servers ", 2,
