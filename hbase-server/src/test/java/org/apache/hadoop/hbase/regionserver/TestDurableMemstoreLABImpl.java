...
public class TestDurableMemstoreLABImpl {

  @ClassRule
  public static final HBaseClassTestRule CLASS_RULE =
      HBaseClassTestRule.forClass(TestDurableMemstoreLABImpl.class);
  ...
  public void testLABLargeAllocation() {
    ...
    List<Cell> list = new ArrayList<Cell>();
    list.add(kv);
    Cell newCell = ((DurableMemStoreLABImpl) mslab).copyCellsInto(list).get(0);
    list.clear();
  ...
  public void testMultiChunkCase() throws Exception {
    ...
    threads[0] = new CopyThread(cells, mslab);
    cells = new ArrayList<Cell>();
    cells.add(new KeyValue(rk, cf, q, new byte[150]));
    cells.add(new KeyValue(rk, cf, q, new byte[40]));
    cells.add(new KeyValue(rk, cf, q, new byte[60]));
    threads[1] = new CopyThread(cells, mslab);
  ...
  private static class CopyThread extends Thread {
    List<Cell> cells;
    DurableMemStoreLABImpl mslab;

    public CopyThread(List<Cell> cells, DurableMemStoreLABImpl mslab) {
      this.cells = cells;
      this.mslab = mslab;
    }

    public void run() {
      List<Cell> copyCellsInto = mslab.copyCellsInto(cells);
      System.out.println();
    }
  }
  ...
  public void testLABThreading() throws Exception {
    ...
    final MemStoreLAB mslab = new DurableMemStoreLABImpl();
    ...
    Thread thread = new Thread() {
      ...
      public void run() {
        while (!stopped) {
          ...
          list.add(cellToCopyInto);
          ((DurableMemStoreLABImpl)mslab).copyCellsInto(list);
          list.clear();
        }
      }
